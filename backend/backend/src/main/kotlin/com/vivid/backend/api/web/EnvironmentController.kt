...
class EnvironmentController(
    ...
    ): org.springframework.data.domain.Page<EnvironmentDto> = environmentService.search(q, pageable).map { it.toDto() }

    @GetMapping("/all")
    @Operation(summary = "Get all environments (non-paginated)")
    fun all(): List<EnvironmentDto> = environmentService.getAll().map { it.toDto() }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    @Operation(summary = "Create a new environment")
    fun create(@RequestBody request: EnvironmentCreateRequest): EnvironmentDto = environmentService.create(request).toDto()
